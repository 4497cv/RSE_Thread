...
#define APP_TEAM5_URI_PATH 						"/team5" /**** URI PATH ***/
...
)
{
    ...
    /* Create CoAP instance for all the parameters */
...
)
{
    ...
    pCountString = App_GetCounterData();
    ackPloadSize = strlen((char*)pCountString);
    ...
    if(gCoapConfirmable_c == pSession->msgType)
    {
    	...
    	switch(pSession->code)
    	{
			case gCoapGET_c:
				...
				COAP_Send(pSession, gCoapMsgTypeAckSuccessContent_c, pCountString, ackPloadSize);
			break;
			case gCoapPOST_c:
				//shell_write("");

...
/* Timer used to keep the device in Up state for an amount of time */
tmrTimerID_t gAppRetrieveCounterTimer = gTmrInvalidTimerID_c;
...
)
{
    ...
    switch(pEventParams->code)
    {
        ...
        case gThrEv_GeneralInd_Connected_c:
            ...
            TMR_StartTimer(gAppRetrieveCounterTimer, gTmrSingleShotTimer_c, 1000, APP_RetrieveTimerCounterCb, NULL);
            break;
...
)
{
    ...
    /* Keep the device Up for 5 seconds - using a timer */
	 if(gAppRetrieveCounterTimer == gTmrInvalidTimerID_c)
	 {
		 gAppRetrieveCounterTimer = TMR_AllocateTimer();
	 }
